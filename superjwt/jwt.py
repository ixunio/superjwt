...
class JWT:
    def __init__(
        ...
        default_claims_validation: JWTValidationConfig = JWTClaimsDefaultValidation,
    ...
    def encode(
        ...
        *,
        ...
        Args:
            ...
            claims_validation (type[JWTBaseModel] | JWTValidationConfig | Validation | None, opt.):
                Validation configuration for claims. Can be a pydantic model class, a JWTValidationConfig
                instance, Validation.DEFAULT (uses default validation), Validation.DISABLE (no validation),
                or None (no validation).
            headers_validation (type[JOSEHeader] | JWTValidationConfig | Validation | None, opt.):
                Validation configuration for headers. Can be a pydantic model class, a JWTValidationConfig
                instance, Validation.DEFAULT (uses default validation), Validation.DISABLE (no validation),
                or None (no validation).
        ...
        if claims is None:
            claims = JWTBaseModel()
        claims_validation_config = self.get_claims_validation_config(
            claims, claims_validation
        )

        try:
            claims_dict = claims_validation_config.run(claims)
        ...
        self.jws.token.verified.model.claims = self.get_claims_data_model(
            claims, claims_validation
        ).model_construct(**claims_dict)
    ...
    def decode(
        ...
        *,
        ...
        if with_detached_payload is not None:
            ...
            try:
                claims_dict = claims_validation_config.run(with_detached_payload)
            except ValidationError as e:
                raise ClaimsValidationError(validation_errors=e.errors()) from e
